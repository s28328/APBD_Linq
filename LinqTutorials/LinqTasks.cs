...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<Emp> result = Emps
                .Where(emp => emp.Salary == Emps.Max(empMax => empMax.Salary));
            return result;
        }
        ...
        {
            IEnumerable<object> result =  Emps.Select<Emp,object>(emp => new
            {
                Nazwisko = emp.Ename,
                Praca = emp.Job
            });
        ...
        {
            Emp result = Emps.Where(emp => emp.Job == "Backend programmer").OrderByDescending(emp => emp.HireDate).First();
            return result;
        }
        ...
        public static IEnumerable<object> Task10()
        {
            IEnumerable<object> result = Emps.Select(emp => new
            {
                ...
                emp.Job,
                emp.HireDate
            }).Concat<object>(new[] { new { Ename = "Brak wartości", Job = (string)null, Hiredate = (DateTime?)null } });
            return result;
        }
        ...
        public static IEnumerable<Dept> Task14()
        {
            IEnumerable<Dept> result = Depts.GroupJoin(Emps,
                ...
                .Where(join => join.EmployeeCount == 0 || join.EmployeeCount == 5)
        ...
        {
            IEnumerable<object> result = Emps.Where(emp => emp.Job.Contains("A"))
                .GroupBy(emp => emp.Job)
        ...
        {
            IEnumerable<object> result = Emps.SelectMany(
                emp => Depts,
                (emp, dept) => new { emp, dept });
            //result =
            return result;
